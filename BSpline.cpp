...
BSpline::BSpline(size_t deg_u, size_t deg_v,
    ...
    : nu(_nu), nv(_nv), control_points(cpts)
{
    basis_u_.setDegree(deg_u);
    basis_v_.setDegree(deg_v);
...
BSpline::eval(double u, double v) const {
    ...
    for (size_t i = 0; i <= p_u; ++i) {
        ...
        for (size_t j = 0; j <= p_v; ++j) {
            int size = base + span_v - p_v + j;
            Vec c = control_points[size];
            point += c * coeff_u[i] * coeff_v[j];
...
void BSpline::draw() {
    ...
    for (size_t k = 0; k < 2; ++k)
        for (size_t i = 0; i <= numControlPoints()[k]-1; ++i) {
            glBegin(GL_LINE_STRIP);
            for (size_t j = 0; j <= numControlPoints()[1-k]-1; ++j) {
                size_t index = k ? j * m + i : i * m + j;
    ...
    glBegin(GL_POINTS);
        ...
        for (size_t i = 0; i < numControlPoints()[0]; ++i) {
            for (size_t j = 0; j < numControlPoints()[1]; ++j) {
                if(i == 0 || j == 0 || i == numControlPoints()[0] - 1
                    || j == numControlPoints()[1] - 1){
                const auto& p = controlPoint(i, j);
                glVertex3dv(p);
                }
            }
        }
...
void BSpline::arrow() {
    for (size_t i = 0; i < numControlPoints()[0]; ++i) {
        for (size_t j = 0; j < numControlPoints()[1]; ++j) {
            if (i == 0 || j == 0 || i == numControlPoints()[0] - 1
                || j == numControlPoints()[1] - 1) {
                Vec const& p = controlPoint(i, j);
                glPushName(index(i,j));
                glRasterPos3fv(p);
                glPopName();
            }
        }
...
void BSpline::calculate_points()
{
    ...
    for (size_t i = 0; i < numControlPoints()[0]; ++i) {
        for (size_t j = 0; j < numControlPoints()[1]; ++j) {
            ...
            if (i != 0 && j != 0 && i != numControlPoints()[0] - 1
                && j != numControlPoints()[1] - 1) {
                control_points[ind] = 0.5 * ((1 - yi(i)) * control_points[index(0, j)] + yi(i) * control_points[index(nu, j)] +
                    (1 - yj(j)) * control_points[index(i, 0)] + yj(j) * control_points[index(i, nv)]);
            }
        }
    }
    for (int k = 0; k < 1000; k++)
    {
        ...
        for (size_t i = 0; i < numControlPoints()[0]; ++i) {
            for (size_t j = 0; j < numControlPoints()[1]; ++j) {
                ...
                if (i != 0 && j != 0 && i != numControlPoints()[0] - 1
                    && j != numControlPoints()[1] - 1) {
                    auto op = control_points[ind];
                    float u_delta_neg = (yi(i) - yi(i - 1)) / (yi(i + 1) - yi(i - 1));
                    float u_delta_pos = (yi(i + 1) - yi(i)) / (yi(i + 1) - yi(i - 1));
                    float v_delta_neg = (yj(j) - yj(j - 1)) / (yj(j + 1) - yj(j - 1));
                    float v_delta_pos = (yj(j + 1) - yj(j)) / (yj(j + 1) - yj(j - 1));
                    control_points[ind] = f * (u_delta_pos * control_points[index(i - 1, j)] + u_delta_neg * control_points[index(i + 1, j)] + v_delta_pos * control_points[index(i, j - 1)] + v_delta_neg * control_points[index(i, j + 1)]) +
                        (1 - 2 * f) * (u_delta_pos * v_delta_pos * control_points[index(i - 1, j - 1)] + u_delta_pos * v_delta_neg * control_points[index(i - 1, j + 1)] + u_delta_neg * v_delta_pos * control_points[index(i + 1, j - 1)] + u_delta_neg * v_delta_neg * control_points[index(i + 1, j + 1)]);

                    if (distance(op, control_points[ind]) > max)
                    {
                        max=distance(op, control_points[ind]);
                    }
...
BSpline BSpline::open(const std::string& filname)
{
